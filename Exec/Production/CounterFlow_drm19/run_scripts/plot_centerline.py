...
def main():
    ...
    for solver in solvers:
        ...
        if t_wall is not None:
            timings[solver] = (t_wall, n_steps)
            print(f"           timing: {t_wall:.1f} s  ({n_steps} steps)")
        else:
            print(f"           timing: log not found ({log_path.name})")
    ...
    if timings:
        ...
        # label each bar with the value and step count
        ...
        ax_time.set_title("Total CPU time per solver", fontsize=13)
